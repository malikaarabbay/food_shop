            ...
            <div class="row justify-content-between">
                <div class="col-lg-4 col-sm-8 col-md-6">
                    <div class="fp__footer_content">
                        ...
                        @if (config('settings.footer_info') != NULL)
                        <span>{{ config('settings.footer_info') }}</span>
                        @endif
                        @if (config('settings.footer_address') != NULL)
                        <p class="info"><i class="far fa-map-marker-alt"></i>{{ config('settings.footer_address') }}</p>
                        @endif
                        @if (config('settings.footer_phone') != NULL)
                        <a class="info" href="callto:{{ config('settings.footer_phone') }}"><i class="fas fa-phone-alt"></i>{{ config('settings.footer_phone') }}</a>
                        @endif
                        @if (config('settings.footer_email') != NULL)
                        <a class="info" href="mailto:{{ config('settings.footer_email') }}"><i class="fas fa-envelope"></i>
                            {{ config('settings.footer_email') }}</a>
                        @endif
                    </div>
                </div>
