                                ...
                                @foreach ($senders as $sender)
                                    ...
                                    <li class="media fp_chat_user" data-name="{{ $chatUser->name }}" data-user="{{ $chatUser->id }}" style="cursor: pointer">
                                        <img alt="image" class="mr-3 rounded-circle " width="50"
                                             src="{{ asset($chatUser->image) }}" style="width: 50px;height: 50px; object-fit: cover;">
...
@push('scripts')
    <script>
        $(document).ready(function(){
            ...
            var avatar = "{{ asset(auth()->user()->image) }}";
            $('#receiver_id').val("");
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, message){
                            let avatar = "{{ asset(':avatar') }}".replace(':avatar', message.sender.image);
                            let html = `
                        ...
                        scrollToBootom()
                        $(".message-envelope").removeClass("beep");
                    },
